...
# My custom modules
from modules.utils import extend_instance
from modules.megatron_gpt_model import EmbeddingScalingMixin, MegatronGPTModel, get_specs
from modules.module import Float16Module
from modules.megatron_utils import average_losses_across_data_parallel_group
from modules.megatron_trainer_builder import MegatronTrainerBuilder
from modules.nlp_overrides import NLPSaveRestoreConnector
from modules import hydra_runner
from modules.exp_manager import exp_manager
from modules.model_utils import load_config, unwrap_model

mp.set_start_method("spawn", force = True)

# Below are the model config fields which affect the structure of the model. These will be the values taken from the teacher model's config file,
# while the rest remain the same as the student model.
...
class DistillationMegatronGPTModel(MegatronGPTModel):
    ...
    def get_forward_output_and_loss_func(self, validation_step = False, tuning = False):
        def fwd_output_and_loss_func(dataloader_iter, model, checkpoint_activations_all_layers = None):
            ...
            ### TO-DO: Take a look at inner attention
            required_keys = set()
    ...
    def loss_func(self, loss_mask, num_valid_tokens_in_ub, output_tensor, validation_step = False):
        ...
        if not validation_step and self.cfg.tensor_model_parallel_size > 1:    
            # KD loss needs extra all-reduce to ensure same values across MP-TP partitions.
...
class LogitsKLLoss(BaseLoss):
    ...
    def __init__(
        self,
        tensor_parallel: bool = False,          # Whether tensor parallelism is enabled or not.
        temperature: float = 1.0,               # Divide tensors by this value prior to calculating loss.
        reverse: bool = False,                  # Whether to reverse the loss as KLD(teacher, student) instead of KLD(student, teacher)
    ):
    ...
    def forward(self,           # Forward function
                predictions: Tensor,           # Student model tensors (size [s, b, h])
                targets: Tensor                # Teacher model tensors (size [s, b, h])
                ) -> Tensor:
        ...
        return self.post_forward(loss)               # KLD loss of tensors (size [b, s])
...
# Function for overwriting model-architecture fields of a config with a checkpoint.
